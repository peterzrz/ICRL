...
class Human():
    ...
    def update_human_human_trust(self, other_h):
        self.trust[other_h.name] = (self.trust[other_h.name][0] + self.ws_hh * other_h.performance, 
                                    self.trust[other_h.name][1] + self.wf_hh * (1-other_h.performance))
...
class Robot():
    def __init__(self, name, objective=0, kappa1=kappa1, kappa2=kappa2, sensedTB=0):
        self.name = name
        self.kappa1 = kappa1
        self.kappa2 = kappa2
    ...
    def assess_danger(self, d):
        self.d_hat = np.random.beta(d*self.kappa2, (1-d)*self.kappa2)
...
f = open("testHistory_revised.txt", 'w')
